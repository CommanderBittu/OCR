...
function ResultPage() {
  ...
  const { text = '', imageUrl = null, processed_image = null } = location.state || {};
  ...
  useEffect(() => {
    if (processed_image) {
        console.log("ProcessedImage is available. Length:", processed_image.length);
    } else {
        console.log("No ProcessedImage available.");
    }
}, [processed_image]);
  useEffect(() => {
